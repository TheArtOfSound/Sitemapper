...
const USER_AGENT = 'SitemapperWorker/0.3 (+https://github.com/TheArtOfSound/Sitemapper)';
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/api/report') {
      const site = url.searchParams.get('site') || '';
      if (!site) return html('<h1>Missing site parameter.</h1>', 400);
      try {
        const result = await analyzeSite(site);
        await incrementStats(env, result.stats.pages);
        return html(renderReport(result));
      } catch (error) {
        return html(`<h1>Sitemapper report failed</h1><p>${escapeHtml(error instanceof Error ? error.message : 'Analysis failed.')}</p>`, 500);
      }
    }
...
async function analyzeSite(input: string): Promise<SitemapperResult> {
  ...
  if (!source.discoveredFromRobots) rootIssues.push({ severity: 'notice', code: 'ROBOTS_NO_SITEMAP_REFERENCE', message: 'robots.txt did not expose a Sitemap: entry; tried /sitemap.xml fallback.' });
  for (const failed of sitemapLoad.failedSitemaps) rootIssues.push({ severity: 'warning', code: 'SITEMAP_FETCH_FAILED', message: `Could not load sitemap: ${failed}` });
  if (sitemapLoad.entries.length > MAX_PAGES) rootIssues.push({ severity: 'notice', code: 'SAMPLE_LIMIT_REACHED', message: `Analyzed ${MAX_PAGES} sample pages from ${sitemapLoad.entries.length} sitemap URLs.` });
  if (sitemapLoad.entries.length === 0) rootIssues.push({ severity: 'error', code: 'NO_SITEMAP_URLS_FOUND', message: 'No URLs were found in the discovered sitemap files.' });

  const inspected = await inspectPages(sitemapLoad.entries.slice(0, MAX_PAGES));
  const pages = dedupePages(inspected, rootIssues);
  const duplicateIssues = findDuplicateMetadataIssues(pages);
  const pagesWithDuplicates = pages.map((page) => ({ ...page, issues: [...page.issues, ...(duplicateIssues.get(page.url) ?? [])] }));

  return {
    site,
    generatedAt: new Date().toISOString(),
    source: { ...source, sitemapUrls: sitemapLoad.loadedSitemaps.length ? sitemapLoad.loadedSitemaps : source.sitemapUrls },
...
async function discoverSitemaps(site: string): Promise<{ robotsUrl: string; sitemapUrls: string[]; discoveredFromRobots: boolean }> {
  ...
  try {
    ...
    const sitemapUrls = extractSitemapUrls(robots.text).filter((u) => sameHost(u, site));
    if (sitemapUrls.length) return { robotsUrl, sitemapUrls, discoveredFromRobots: true };
  } catch {}
...
async function loadSitemapEntries(site: string, initialUrls: string[]): Promise<{ entries: RawSitemapEntry[]; loadedSitemaps: string[]; failedSitemaps: string[] }> {
  ...
  const queue = initialUrls.filter((u) => sameHost(u, site));
  ...
  while (queue.length && seen.size < MAX_SITEMAPS && entries.size < MAX_URLS) {
    ...
    try {
      ...
      for (const item of asArray(xml?.sitemapindex?.sitemap)) {
        const loc = textValue(item?.loc);
        if (loc && sameHost(loc, site) && !seen.has(normalizePageUrl(loc))) queue.push(loc);
      }
      for (const item of asArray(xml?.urlset?.url)) {
        const loc = textValue(item?.loc);
        if (!loc || !sameHost(loc, site)) continue;
        const key = normalizePageUrl(loc);
        if (!entries.has(key)) entries.set(key, { url: key, lastmod: textValue(item?.lastmod), changefreq: textValue(item?.changefreq), priority: textValue(item?.priority) });
...
async function inspectPages(entries: RawSitemapEntry[]): Promise<PageRecord[]> {
  ...
  for (const batch of chunk(entries, 8)) {
    const pages = await Promise.all(batch.map(async (entry) => {
      try {
...
function buildPage(entry: RawSitemapEntry, status: number | undefined, finalUrl: string | undefined, htmlText: string | undefined): PageRecord {
  ...
  const title = htmlText ? cleanText(matchFirst(htmlText, /<title[^>]*>([\s\S]*?)<\/title>/i)) : undefined;
  const description = htmlText ? cleanText(matchMeta(htmlText, 'description')) : undefined;
  const canonical = htmlText ? cleanText(matchCanonical(htmlText)) : undefined;
  const robots = htmlText ? cleanText(matchMeta(htmlText, 'robots'))?.toLowerCase() : undefined;
  ...
  if (finalUrl && normalizePageUrl(finalUrl) !== normalizePageUrl(entry.url)) issues.push({ severity: 'warning', code: 'REDIRECTED_URL', message: `Sitemap URL resolves to ${finalUrl}.` });
  ...
  return { url: entry.url, path, displayPath, section, pageType, title, description, canonical, finalUrl, lastmod: entry.lastmod, changefreq: entry.changefreq, priority: entry.priority, status, ok: issues.every((i) => i.severity !== 'error'), issues };
}
...
async function fetchText(url: string): Promise<{ status: number; finalUrl: string; text: string }> {
  ...
  try {
    const response = await fetch(url, { redirect: 'follow', signal: controller.signal, headers: { 'user-agent': USER_AGENT, accept: 'text/html,application/xhtml+xml,application/xml,text/xml;q=0.9,*/*;q=0.6' } });
...
function dedupePages(pages: PageRecord[], rootIssues: PageIssue[]): PageRecord[] {
  ...
  for (const page of pages) {
    ...
    else { duplicates += 1; byKey.set(key, completeness(page) > completeness(existing) ? page : existing); }
  }
...
function stats(pages: PageRecord[], rootIssues: PageIssue[]): SitemapperStats {
  ...
  return { pages: pages.length, sections: new Set(pages.map((p) => p.section)).size, errors: all.filter((i) => i.severity === 'error').length, warnings: all.filter((i) => i.severity === 'warning').length, notices: all.filter((i) => i.severity === 'notice').length };
}
...
function score(pages: PageRecord[], rootIssues: PageIssue[]): SitemapperScores {
  ...
  const missingTitle = pages.reduce((sum, p) => sum + (!p.title ? pageWeight(p) : 0), 0);
  const missingDescription = pages.reduce((sum, p) => sum + (!p.description ? pageWeight(p) : 0), 0);
  ...
  return { index: clamp(100 - Math.round((missingTitle / totalWeight) * 20) - Math.max(0, 10 - sections) * 2), seo: clamp(100 - errors * 10 - warnings * 1.25 - notices * 0.15 - Math.round((missingDescription / totalWeight) * 12)), sitemap: clamp(100 - rootIssues.filter((i) => i.severity === 'error').length * 15 - Math.round((missingLastmod / totalWeight) * 10)) };
}

function renderReport(result: SitemapperResult): string {
  const host = new URL(result.site).hostname;
  const issueRows = topIssues(result).map(([code, count]) => `<tr><td>${escapeHtml(humanize(code))}</td><td>${count}</td></tr>`).join('');
  const pageRows = result.pages.slice(0, 80).map((p) => `<tr><td><a href="${escapeAttr(p.url)}">${escapeHtml(p.title || p.displayPath)}</a><br><small>${escapeHtml(p.url)}</small></td><td>${escapeHtml(humanize(p.pageType))}</td><td>${p.status || '—'}</td><td>${p.issues.length}</td><td>${escapeHtml(p.issues.slice(0, 4).map((i) => humanize(i.code)).join(', ') || 'Clean')}</td></tr>`).join('');
  return `<!doctype html><html><head><meta charset="utf-8"><title>Sitemapper SEO Report — ${escapeHtml(host)}</title><style>body{font:13px Arial,sans-serif;color:#202124;margin:34px;line-height:1.45}h1{font-size:30px;margin:0 0 4px}h2{font-size:18px;margin-top:28px;border-bottom:1px solid #ddd;padding-bottom:6px}.muted{color:#5f6368}.scores{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:18px 0}.score{border:1px solid #ddd;border-radius:8px;padding:14px;text-align:center}.score strong{font-size:28px;display:block}table{width:100%;border-collapse:collapse;margin-top:10px}th,td{border-bottom:1px solid #eee;text-align:left;vertical-align:top;padding:8px}th{background:#f8f9fa}small{color:#5f6368}.actions{margin:18px 0}.button{border:1px solid #1a73e8;background:#1a73e8;color:#fff;padding:8px 12px;border-radius:4px;text-decoration:none}@media print{.actions{display:none}body{margin:18px}}</style></head><body><h1>Sitemapper SEO Specialist Report</h1><p class="muted">${escapeHtml(host)} · Generated ${escapeHtml(result.generatedAt)} · Sample limit ${MAX_PAGES} pages</p><div class="actions"><button onclick="print()" class="button">Print / Save as PDF</button></div><section class="scores"><div class="score"><strong>${result.scores.index}</strong>Index Score</div><div class="score"><strong>${result.scores.seo}</strong>SEO Score</div><div class="score"><strong>${result.scores.sitemap}</strong>Sitemap Score</div></section><h2>Executive Summary</h2><p>Analyzed ${result.stats.pages} pages across ${result.stats.sections} sections. Found ${result.stats.errors} errors, ${result.stats.warnings} warnings, and ${result.stats.notices} notices. This report is designed for handoff to an SEO specialist, developer, or AI assistant.</p><h2>Discovered Sources</h2><ul>${result.source.sitemapUrls.map((u) => `<li>${escapeHtml(u)}</li>`).join('')}</ul><h2>Top Issue Types</h2><table><thead><tr><th>Issue</th><th>Count</th></tr></thead><tbody>${issueRows || '<tr><td>No major issues detected.</td><td>0</td></tr>'}</tbody></table><h2>Recommended Priority</h2><ol><li>Fix all error-level pages first: failed fetches, bad statuses, and noindex pages listed in sitemaps.</li><li>Review warning-level static pages before generated/archive/cluster pages.</li><li>Consolidate duplicate titles and duplicate meta descriptions where pages are intended to rank separately.</li><li>Add missing lastmod values to sitemap entries where freshness matters.</li><li>Use the page list below as a technical task queue.</li></ol><h2>Page-Level Findings</h2><table><thead><tr><th>Page</th><th>Type</th><th>Status</th><th>Issues</th><th>Top Findings</th></tr></thead><tbody>${pageRows}</tbody></table></body></html>`;
}

function topIssues(result: SitemapperResult): [string, number][] {
...
function appHtml(): string {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Sitemapper</title><style>${css()}</style></head><body><header><b><span>S</span><span>i</span><span>t</span><span>e</span><span>m</span>apper</b><nav><span id="counts">stats unavailable</span><a href="https://github.com/TheArtOfSound/Sitemapper">GitHub</a></nav></header><section id="home"><h1><span>S</span><span>i</span><span>t</span><span>e</span><span>m</span>apper<small>real sitemap index + SEO checker</small></h1><form id="form" class="box"><i>⌕</i><input id="site" value="https://wesearch.press"><button id="mapBtn" type="submit">Map Site</button></form><p>Enter a public website. Sitemapper fetches robots.txt, sitemap indexes, URLs, page metadata, and SEO issues server-side.</p><p class="examples"><button type="button" data-site="https://wesearch.press">wesearch.press</button> · <button type="button" data-site="https://example.com">example.com</button></p><p id="homeStatus" class="muted"></p></section><section id="app"><div class="search"><b>Sitemapper</b><form id="form2" class="box small"><i>⌕</i><input id="site2"><button id="mapBtn2" type="submit">Map</button></form></div><div class="tabs"><button data-filter="all" class="active">All</button><button data-filter="static">Static</button><button data-filter="category">Categories</button><button data-filter="generated">Generated</button><button data-filter="errors">Errors</button><button data-filter="warnings">Warnings</button><button data-filter="clean">Clean</button></div><main><section><p id="status">Waiting.</p><div id="progress"><div><span id="plabel">Starting</span><span id="pct">0%</span></div><em><i id="bar"></i></em><div id="steps"></div></div><div id="results"></div></section><aside><div class="panel"><h2>Scores</h2><div class="scores"><strong id="si">—</strong><strong id="ss">—</strong><strong id="sm">—</strong><small>Index</small><small>SEO</small><small>Sitemap</small></div></div><div class="panel"><h2>Run</h2><p id="run">No run yet.</p></div><div class="panel"><h2>Exports</h2><p><a id="reportLink" class="button" href="#" target="_blank" rel="noopener">SEO Specialist Report</a></p><p><a id="jsonLink" href="#" target="_blank" rel="noopener">Raw JSON</a></p></div><div class="panel"><h2>Console</h2><pre id="console">$ waiting</pre></div></aside></main></section><script>${js()}</script></body></html>`;
}

function css(): string { return `body{margin:0;background:#fff;color:#202124;font:13px Arial,Helvetica,sans-serif}a{color:#1a0dab;text-decoration:none}a:hover{text-decoration:underline}header{height:44px;border-bottom:1px solid #eee;display:flex;align-items:center;justify-content:space-between;padding:0 18px;color:#5f6368}header b span:nth-child(1),h1 span:nth-child(1){color:#4285f4}header b span:nth-child(2),h1 span:nth-child(2){color:#ea4335}header b span:nth-child(3),h1 span:nth-child(3){color:#fbbc05}header b span:nth-child(4),h1 span:nth-child(4){color:#4285f4}header b span:nth-child(5),h1 span:nth-child(5){color:#34a853}nav{display:flex;gap:14px}#home{max-width:760px;margin:80px auto;text-align:center;padding:0 16px}h1{font-size:58px;letter-spacing:-3px;margin:0 0 22px;font-weight:500}h1 small{display:block;font-size:12px;letter-spacing:0;color:#5f6368;margin-top:8px}.box{height:46px;display:flex;align-items:center;border:1px solid #dfe1e5;border-radius:24px;box-shadow:0 1px 6px rgba(32,33,36,.14);padding:0 10px;background:#fff}.box input{border:0;outline:0;flex:1;font-size:16px}.box button,.button{border:1px solid #1a73e8;background:#1a73e8;color:#fff;border-radius:4px;padding:8px 12px;cursor:pointer;text-decoration:none}.box i{color:#9aa0a6;padding:0 8px}.examples button,.tabs button{border:0;background:transparent;color:#1a0dab;cursor:pointer}.muted{color:#70757a}#app{display:none}.search{border-bottom:1px solid #ebebeb;padding:14px 22px;display:grid;grid-template-columns:140px minmax(300px,760px);gap:18px;align-items:center}.small{height:40px;box-shadow:none}.tabs{margin-left:180px;padding:10px 0;border-bottom:1px solid #f1f3f4;display:flex;gap:20px}.tabs button{color:#5f6368}.tabs .active{color:#1a73e8;font-weight:bold}main{display:grid;grid-template-columns:minmax(320px,760px) 300px;gap:34px;margin-left:180px;margin-right:24px;padding-top:14px}#status{color:#70757a}#progress{display:none;border:1px solid #dadce0;border-radius:6px;padding:12px;margin-bottom:16px}#progress>div:first-child{display:flex;justify-content:space-between}em{display:block;height:4px;background:#eee;border-radius:99px;overflow:hidden;margin:8px 0}em i{display:block;height:100%;background:#1a73e8;width:0;transition:.25s}.step{display:inline-block;margin:4px 14px 0 0;color:#5f6368}.done{color:#188038}.result{padding:12px 0 15px;border-bottom:1px solid #f1f3f4;animation:in .22s both}.result h3{font-size:18px;font-weight:400;margin:0 0 2px}.url{color:#006621;word-break:break-all}.meta{color:#70757a;font-size:12px;margin-top:3px}.snippet{color:#4d5156;margin-top:4px}.badge{font-size:11px;border:1px solid #e1e4e8;background:#f1f3f4;border-radius:3px;padding:2px 5px;margin-right:4px}.error{color:#b00020}.warning{color:#8a5a00}.notice{color:#1a73e8}.panel{border:1px solid #dadce0;border-radius:8px;padding:12px;margin-bottom:12px}.panel h2{font-size:16px;font-weight:400;margin:0 0 8px}.scores{display:grid;grid-template-columns:1fr 1fr 1fr;text-align:center;gap:6px}.scores strong{font-size:22px}.scores small{color:#70757a}pre{background:#fafafa;border:1px solid #eee;border-radius:5px;padding:8px;max-height:180px;overflow:auto;white-space:pre-wrap}@keyframes in{from{opacity:0;transform:translateY(4px)}to{opacity:1;transform:none}}@media(max-width:900px){.search{grid-template-columns:1fr}.tabs,main{margin-left:16px;margin-right:16px}main{display:block}h1{font-size:46px}}`; }

function js(): string { return `const $=id=>document.getElementById(id);let data=null,filter='all';function init(){fetch('/api/stats').then(r=>r.json()).then(s=>{$('counts').textContent=s.runs.toLocaleString()+' tries · '+s.pages.toLocaleString()+' pages mapped'}).catch(()=>{$('counts').textContent='stats unavailable'});$('form').addEventListener('submit',e=>{e.preventDefault();run($('site').value)});$('form2').addEventListener('submit',e=>{e.preventDefault();run($('site2').value)});$('mapBtn').addEventListener('click',e=>{e.preventDefault();run($('site').value)});$('mapBtn2').addEventListener('click',e=>{e.preventDefault();run($('site2').value)});document.querySelectorAll('[data-site]').forEach(b=>b.onclick=()=>run(b.dataset.site));document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-filter]').forEach(x=>x.classList.remove('active'));b.classList.add('active');filter=b.dataset.filter;render()});$('homeStatus').textContent='Ready.'}document.readyState==='loading'?document.addEventListener('DOMContentLoaded',init):init();async function run(site){site=norm(site);$('homeStatus').textContent='Processing '+site+'...';$('home').style.display='none';$('app').style.display='block';$('site2').value=site;$('results').innerHTML='';$('console').textContent='$ sitemapper analyze '+site+'\n';$('status').textContent='Starting real server-side analysis...';$('mapBtn').disabled=true;$('mapBtn2').disabled=true;$('mapBtn').textContent='Processing...';$('mapBtn2').textContent='Processing...';$('progress').style.display='block';let steps=['normalize domain','fetch robots.txt','discover sitemap index','parse sitemap files','classify page types','sample metadata','score SEO checks','render results'];$('steps').innerHTML=steps.map((s,i)=>'<span id=s'+i+' class=step>○ '+s+'</span>').join('');let tick=animateSteps(steps);try{let r=await fetch('/api/analyze?site='+encodeURIComponent(site),{cache:'no-store'});let j=await r.json().catch(()=>({error:'API did not return JSON'}));if(!r.ok||j.error)throw new Error(j.error||('HTTP '+r.status));data=j;clearInterval(tick);completeSteps(steps);$('status').textContent='About '+j.pages.length.toLocaleString()+' results for '+new URL(j.site).hostname;$('si').textContent=j.scores.index;$('ss').textContent=j.scores.seo;$('sm').textContent=j.scores.sitemap;$('run').textContent=j.stats.pages+' pages · '+j.stats.sections+' sections · '+j.stats.errors+' errors · '+j.stats.warnings+' warnings';$('reportLink').href='/api/report?site='+encodeURIComponent(site);$('jsonLink').href='/api/analyze?site='+encodeURIComponent(site);log('✓ '+j.pages.length+' pages ready');$('progress').style.display='none';render()}catch(e){clearInterval(tick);$('status').textContent='Analysis failed: '+e.message;log('✕ '+e.message);$('progress').style.display='none'}finally{$('mapBtn').disabled=false;$('mapBtn2').disabled=false;$('mapBtn').textContent='Map Site';$('mapBtn2').textContent='Map'}}function animateSteps(steps){let i=0;return setInterval(()=>{let n=Math.min(i,steps.length-1);$('plabel').textContent=steps[n];$('pct').textContent=Math.round((n+1)/steps.length*100)+'%';$('bar').style.width=Math.round((n+1)/steps.length*100)+'%';let el=$('s'+n);if(el){el.className='step done';el.textContent='✓ '+steps[n]}log('→ '+steps[n]);i++},260)}function completeSteps(steps){steps.forEach((s,i)=>{let el=$('s'+i);if(el){el.className='step done';el.textContent='✓ '+s}});$('pct').textContent='100%';$('bar').style.width='100%'}function render(){if(!data)return;let pages=data.pages.filter(match).slice(0,120);$('results').innerHTML=pages.map((p,i)=>'<article class=result style="animation-delay:'+Math.min(i*10,200)+'ms"><h3><a href="'+esc(p.url)+'">'+esc(p.title||'(missing title)')+'</a></h3><div class=url>'+esc(p.url)+'</div><div class=meta>'+esc(p.status||'—')+' · '+human(p.pageType)+' · '+esc(p.lastmod||'no lastmod')+' · '+(p.issues||[]).length+' issues</div><div class=snippet>'+esc(p.description||'No meta description found. Sitemapper still indexes this page and reports the missing field.')+'</div><p>'+badges(p)+'</p></article>').join('')||'<p>No matching pages.</p>'}function match(p){if(filter==='all')return true;if(filter==='static')return ['home','static'].includes(p.pageType);if(filter==='category')return ['category','category_page'].includes(p.pageType);if(filter==='generated')return ['generated','cluster','archive','canvas','story'].includes(p.pageType);if(filter==='errors')return (p.issues||[]).some(i=>i.severity==='error');if(filter==='warnings')return (p.issues||[]).some(i=>i.severity==='warning');if(filter==='clean')return !(p.issues||[]).length;return true}function badges(p){return !(p.issues||[]).length?'<span class=badge>clean</span>':p.issues.slice(0,5).map(i=>'<span class="badge '+i.severity+'">'+human(i.code)+'</span>').join(' ')}function log(t){$('console').textContent+=t+'\n';$('console').scrollTop=$('console').scrollHeight}function norm(v){v=(v||'').trim();return /^https?:\/\//i.test(v)?v:'https://'+v}function human(v){return String(v||'').replace(/_/g,' ').replace(/\b\w/g,c=>c.toUpperCase())}function esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}`; }

function completeness(page: PageRecord): number { return Number(Boolean(page.title)) * 3 + Number(Boolean(page.description)) * 2 + Number(Boolean(page.lastmod)) + Number(page.status === 200) * 2; }
function duplicateSeverity(page: PageRecord): 'warning' | 'notice' { return ['cluster', 'archive', 'category_page', 'canvas', 'generated'].includes(page.pageType) ? 'notice' : 'warning'; }
...
function pageWeight(page: PageRecord): number { if (['home', 'static'].includes(page.pageType)) return 1; if (['category', 'source'].includes(page.pageType)) return 0.75; if (['category_page', 'archive', 'canvas'].includes(page.pageType)) return 0.35; if (['cluster', 'story', 'generated'].includes(page.pageType)) return 0.25; return 0.5; }
...
function normalizeSite(input: string): string { const withProtocol = /^https?:\/\//i.test(input) ? input : `https://${input}`; const url = new URL(withProtocol); url.hash = ''; url.search = ''; url.pathname = url.pathname === '/' ? '' : url.pathname.replace(/\/+$/, ''); return url.toString().replace(/\/$/, ''); }
...
function matchFirst(text: string, regex: RegExp): string | undefined { return regex.exec(text)?.[1]; }
function matchMeta(text: string, name: string): string | undefined { const a = new RegExp(`<meta[^>]+name=["']${name}["'][^>]+content=["']([^"']*)["'][^>]*>`, 'i'); const b = new RegExp(`<meta[^>]+content=["']([^"']*)["'][^>]+name=["']${name}["'][^>]*>`, 'i'); return a.exec(text)?.[1] || b.exec(text)?.[1]; }
function matchCanonical(text: string): string | undefined { return /<link[^>]+rel=["']canonical["'][^>]+href=["']([^"']*)["'][^>]*>/i.exec(text)?.[1] || /<link[^>]+href=["']([^"']*)["'][^>]+rel=["']canonical["'][^>]*>/i.exec(text)?.[1]; }
...
function humanize(value: string): string { return String(value || '').replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()); }
...
function json(data: unknown, status = 200): Response { return withCors(new Response(JSON.stringify(data), { status, headers: { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' } })); }
